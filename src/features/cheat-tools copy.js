Vue.component('fake-website',{
  ...
  methods:{
    ...
    // 快速增加金钱、知识
    onFastChange(val){
      localStorage.setItem('fastPlusResource',+val)
      if(+val){
        this.startFastPlus()
      }else{
        this.stopFastPlus()
      ...
      setTimeout(()=>{
        ...
        this.fastPlusTimer = setInterval(()=>{
          ...
          Object.keys(global.resource).forEach(key=>{
            if(fastResourceKeys.includes(key) || !global.resource[key].display) return
            if(global.resource[key].max > 0){
              if(global.resource[key].amount <= global.resource[key].max * 0.5){
            ...
            }else if(global.resource[key].max == -2){
              global.resource[key].amount++
            }
          })
          fastResourceKeys.forEach(key=>{
            if(global.resource[key] && global.resource[key].amount < global.resource[key].max){
              if(global.resource[key].amount <= global.resource[key].max * 0.9){
                global.resource[key].amount = global.resource[key].max * 0.9
              }else{
                global.resource[key].amount = Math.min(global.resource[key].amount * 1.5, global.resource[key].max)
              }
            }
          })
        },1000)
